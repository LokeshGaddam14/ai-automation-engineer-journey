    ...
    <div className="glass-card p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-white/40 mt-1">
            <Phone size={12} />
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="text-center p-3 rounded-xl bg-slate-900/5 dark:bg-white/[0.04] border border-slate-900/10 dark:border-white/[0.06]">
          <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{patient.total_calls}</p>
          <p className="text-[11px] text-slate-500 dark:text-white/40 mt-0.5">Total Calls</p>
        </div>
        <div className="text-center p-3 rounded-xl bg-slate-900/5 dark:bg-white/[0.04] border border-slate-900/10 dark:border-white/[0.06]">
          ...
          <p className="text-[11px] text-slate-500 dark:text-white/40 mt-0.5">Last Call</p>
        </div>
        <div className="text-center p-3 rounded-xl bg-slate-900/5 dark:bg-white/[0.04] border border-slate-900/10 dark:border-white/[0.06]">
          <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{patient.last_treatment ?? '—'}</p>
          <p className="text-[11px] text-slate-500 dark:text-white/40 mt-0.5">Last Treatment</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      {history.calls.length === 0 ? (
        <div className="text-center py-10 text-slate-500 dark:text-white/30">
        ...
        <div className="divide-y" style={{ borderColor: 'rgba(16,185,129,0.08)' }}>
          {history.calls.map((call, idx) => (
            <div key={idx} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-900/5 dark:bg-white/[0.03] transition-colors">
              <div className="w-8 h-8 rounded-lg bg-emerald-500/15 flex items-center justify-center flex-shrink-0">
                <Phone size={13} className="text-emerald-400" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-500 dark:text-white/80 truncate">{call.treatment ?? 'General Inquiry'}</p>
                <p className="text-[11px] font-mono text-slate-500 dark:text-white/30 truncate">{call.call_id}</p>
              </div>
              <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-white/40 flex-shrink-0">
  ...
  return (
    <div ref={pageRef} className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-white/40 mt-1">Search patient records by phone number</p>
      </div>

      {/* Search form */}
      <form onSubmit={handleSearch} className="flex gap-3 max-w-lg">
        <div className="relative flex-1">
          <Phone size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 dark:text-white/30" />
          <input
            type="tel"
            value={phone}
      ...
      {!loading && !patient && !error && (
        <div className="glass-card flex flex-col items-center justify-center py-20 text-slate-500 dark:text-white/20">
