...
Setup (FREE — no credit card needed):
    ...
    4. Download JSON → save as: aria/integrations/credentials.json
...
CLINIC_START_HOUR   = int(os.getenv("CLINIC_START_HOUR", "9"))
CLINIC_END_HOUR     = int(os.getenv("CLINIC_END_HOUR",   "18"))
SLOT_DURATION_MINS  = int(os.getenv("SLOT_DURATION_MINS", "30"))
...
class GoogleCalendarAgent:
    ...
    def get_available_slots(self, date_str: str) -> List[str]:
        ...
        Args:
            date_str: "2026-07-11", "tomorrow", "Monday"
    ...
    def _resolve_date(self, date_str: str) -> Optional[datetime]:
        ...
        if date_lower in ("tomorrow", "రేపు", "kal", "நாளை", "कल"):
    ...
    def _normalize_time(self, time_str: str) -> Optional[str]:
        """Convert '10 AM', '10:00 AM', '14:30' → 'HH:MM'."""
        import re
        time_str = time_str.strip().upper()
        time_str = time_str.replace("ఉదయం", "AM").replace("రాత్రి", "PM")
